...
from benchmarks import create_uci_dataloader
...
if __name__ == "__main__":
    ...
    x_expanded = jnp.expand_dims(x, batch_dims_to_expand + (-1,))
    y_expanded = (
        nn.one_hot(y, n_classes)
        if batch_shape == ()
        else jnp.expand_dims(nn.one_hot(y, n_classes), batch_dims_to_expand)
    )

    # train the model on the whole dataset
    model.fit_vmp(
        x_expanded, y_expanded, n_m_steps=args.n_m_steps, compute_accuracy=False
    )
    # Get the predictions of the model on the test set
    logits = model.predict(x_expanded).logits
    ...
    for model_i in range(args.n_models):
        ...
        lpd_i = jnp.sum(logits_model_i * nn.one_hot(y, n_classes), -1).mean()

        lpd_all_models += lpd_i

    waic = lpd_all_models / args.n_models
