...
if __name__ == "__main__":
    ...
    if not os.path.exists("./logging/"):
        os.makedirs("./logging/")
    exp_name = f"pinwheel-bbvi-cmn-layers={num_layers}-n_components={args.n_components}-hidden_dims={hidden_dim}-train_size={args.train_size}-n_classes={args.n_classes}"
    ...
    # In order to vmap model generation over keys, first create a partial'd version of `fit_cmn_bbvi` that
    ...
    # initializes and fits (using gradient descent on the log likelihood of the model) a set of `n_models` independently-initialized CMNs on the training data
    train_acc, test_acc, lpd, ece, res, grid_predicted_class = vmap(
        fit_cmn_bbvi_one_model
    )(n_model_keys)
