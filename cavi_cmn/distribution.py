...
from cavi_cmn import ArrayDict, utils
...
class Distribution:
    ...
    def __init__(self, default_event_dim: int, batch_shape: tuple, event_shape: tuple):
        ...
        self.dim = (
            self.event_shape[-default_event_dim] if len(self.event_shape) > 0 else 0
        )
    ...
    def expand_batch_shape(self, batch_relative_axes: Union[int, Tuple[int]]):
        ...
        absolute_axes = tuple(
            [ax - self.event_dim if ax < 0 else ax for ax in batch_relative_axes]
        )
        ...
        new_batch_shape = [
            1 if ax in adjusted_axes else next(shape_it) for ax in range(new_batch_dim)
        ]
    ...
    def moveaxis(self, source: int, destination: int):
        ...
        data_fields, aux_fields = self.tree_flatten()
        data_fields = jtu.tree_map(
            lambda x: moveaxis_of_leaf(
                x, source, destination, absolute_src, absolute_dest
            ),
            data_fields,
        )
        ...
        destination = normalize_axis_tuple(destination, self.batch_dim, "destination")[
            0
        ]

        # Convert batch_shape to a list to make it mutable
        batch_shape_list = [
            dim for n, dim in enumerate(self.batch_shape) if n != source
        ]
