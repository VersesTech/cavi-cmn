...
from cavi_cmn import Distribution, ExponentialFamily, ArrayDict, utils
...
class Conjugate(Distribution):
    ...
    def expand(self, shape: tuple):
        ...
        self.posterior_params = jtu.tree_map(
            lambda x: jnp.broadcast_to(x, shape_diff + x.shape), self.posterior_params
        )
        self.prior_params = jtu.tree_map(
            lambda x: jnp.broadcast_to(x, shape_diff + x.shape), self.prior_params
        )
    ...
    def kl_divergence(self) -> Array:
        ...
        expected_log_qp = utils.map_and_multiply(
            self.expected_posterior_statistics(), log_qp, self.default_event_dim
        )
    ...
    def update_from_data(
        ...
        weights = (
            self.expand_event_dims(weights) if weights is not None else jnp.ones(shape)
        )
    ...
    def update_from_probabilities(
        ...
        counts_shape = self.get_sample_shape(distribution.mean) + self.get_batch_shape(
            distribution.mean
        )
        shape = counts_shape + (1,) * self.event_dim
        counts = jnp.ones(counts_shape)
    ...
    def sum_stats_over_samples(
        ...
        return jtu.tree_map(
            lambda leaf_array: (leaf_array * weights).sum(sample_dims), stats
        )
