...
from cavi_cmn.utils import map_and_multiply, sum_pytrees, ArrayDict
...
class ExponentialFamily(Distribution):
    ...
    def expand(self, shape: tuple):
        ...
        self.nat_params = jtu.tree_map(
            lambda x: jnp.broadcast_to(x, shape_diff + x.shape), self.nat_params
        )
    ...
    def params_dot_statistics(self, x: Array) -> Array:
        ...
        return map_and_multiply(
            self.nat_params, self.statistics(x), self.default_event_dim, mapping
        )
    ...
    def combine(
        ...
        for other in others:
            if not isinstance(other, self.__class__):
                raise ValueError(
                    "All instances must be of type {}".format(self.__class__.__name__)
                )
